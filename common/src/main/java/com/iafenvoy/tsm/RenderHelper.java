...
public class RenderHelper {
    ...
    static {
        List<? extends EntityType<?>> collect = TsmConfig.getInstance().whitelist.stream().map(Identifier::new).map(Registry.ENTITY_TYPE::get).toList();
        if (collect.isEmpty())
            collect = Registry.ENTITY_TYPE.stream().filter((e) -> !TsmConfig.getInstance().blacklist.contains(Registry.ENTITY_TYPE.getId(e).toString())).toList();
        ALLOW_ENTITIES = collect;
    }

    public static void endClientTick() {
        if (livingEntity == null) {
            Entity entity = ALLOW_ENTITIES.get(RANDOM.nextInt(ALLOW_ENTITIES.size())).create(DummyClientPlayerEntity.DummyClientWorld.getInstance());
